 ...
 * Adds link
 */
add_action( 'dt_list_action_menu_items', 'dt_post_bulk_list_link_messages', 30, 3 );
function dt_post_bulk_list_link_messages( $bulk_send_menu_items, $post_type ) {
    if ( $post_type === 'contacts' || apply_filters( 'dt_bulk_email_connection_field', null, $post_type ) ) {
        $bulk_send_menu_items['bulk-message'] = [
            'label' => __( 'Bulk Message', 'disciple_tools' ),
            'icon' => get_template_directory_uri() . '/dt-assets/images/bulk-edit.svg',
            'section_id' => 'bulk_contact_messaging_picker',
 ...
 */
add_action( 'dt_list_action_section', 'dt_post_bulk_list_section_messages', 30, 3 );
function dt_post_bulk_list_section_messages( $post_type ){
    if ( $post_type !== 'contacts' && !apply_filters( 'dt_bulk_email_connection_field', null, $post_type ) ){
        return;
    }
    ...
    ?>
    <div id="bulk_contact_messaging_picker" class="list_action_section">
